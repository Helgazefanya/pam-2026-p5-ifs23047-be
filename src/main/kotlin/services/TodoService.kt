...
class TodoService(
    ...
    suspend fun getAll(call: ApplicationCall) {
        ...
        // PERBAIKAN: Mengambil total count asli dari database untuk sinkronisasi Dashboard
        val stats = todoRepo.getStats(user.id)
        val totalCount = stats["total"] ?: 0

        // Mengirimkan response lengkap dengan metadata pagination
        val response = DataResponse(
            "success",
            "Berhasil mengambil daftar todo saya",
            mapOf(
                "todos" to todos,
                "total" to totalCount, // Menambahkan total agar di Android tidak dianggap 0
                "page" to page,
                "perPage" to perPage
            )
        )
